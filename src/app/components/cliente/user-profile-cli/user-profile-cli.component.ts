...
@Component({
  selector: 'app-user-profile-cli',
  templateUrl: './user-profile-cli.component.html',
  styleUrl: './user-profile-cli.component.css'
})
export class UserProfileCliComponent implements OnInit{
  usuario: Usuario = {
    id: 0,
    nombre: '',
    email: '',
    ...
    if (this.usuario.id !== undefined) {
      this.authService.updateUsuario(this.usuario.id, this.usuario).subscribe(
        (res: any) => {
          ...
          this.router.navigate(['/pantalla-principal-user']); // Redirige a la página deseada
        },
        (error) => {
