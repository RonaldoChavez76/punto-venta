...
@Component({
  selector: 'app-user-profile-prov',
  templateUrl: './user-profile-prov.component.html',
  styleUrl: './user-profile-prov.component.css'
})
export class UserProfileProvComponent implements OnInit {
  usuario: Usuario = {
    id: 0,
    nombre: '',
    email: '',
    ...
    if (this.usuario.id !== undefined) {
      this.authService.updateUsuario(this.usuario.id, this.usuario).subscribe(
        (res: any) => {
          ...
          this.router.navigate(['/pantalla-principal-prov']); // Redirige a la página deseada
        },
        (error) => {
